...
# -*- coding: utf-8 -*-
"""
Hailo ürünlerini günceller
CSV: "HL 0310-100" → JSON: "0310-100"
"""
...
with open('gorsel_linkleri.csv', 'r', encoding='utf-8-sig') as f:
    ...
    for row in reader:
        ...
        # HAİLO (Türkçe İ harfi ile)
        if 'HA' in marka.upper() and 'LO' in marka.upper() and sku and image_url:
            # SKU normalizasyonu: "HL 0310-100" → "0310-100" (prefix kaldır)
            sku_normalized = sku.replace('HL ', '').strip()
...
print(f"\n✅ {len(image_map)} Hailo ürün resmi bulundu (kayıt: {len(image_map)//2})\n")
...
csv_count = len([k for k in image_map.keys() if k.startswith('HL')])
print(f"\n{'='*60}")
print(f"✅ HAILO TEST TAMAMLANDI!")
print(f"{'='*60}")
print(f"📊 Sonuç:")
...
print(f"\n🎯 Şimdi Hailo markasını aç ve test et!")
