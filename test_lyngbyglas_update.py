...
# -*- coding: utf-8 -*-
"""
Lyngby Glas ürünlerini günceller
CSV: "LG 12050" → JSON: "12050"
"""
...
with open('gorsel_linkleri.csv', 'r', encoding='utf-8-sig') as f:
    ...
    for row in reader:
        ...
        # LYNGBY GLAS
        if 'LYNGBY' in marka.upper() and sku and image_url:
            # SKU normalizasyonu: "LG 12050" → "12050" (prefix kaldır)
            sku_normalized = sku.replace('LG ', '').strip()
...
print(f"\n✅ {len(image_map)} Lyngby Glas ürün resmi bulundu (kayıt: {len(image_map)//2})\n")
...
# Lyngby Glas ürünlerini güncelle
updated = 0
not_found = []
...
for product in products:
    ...
    # "Lyngby Glas" olan ürünler
    if 'LYNGBY' in brand.upper():
...
csv_count = len([k for k in image_map.keys() if k.startswith('LG')])
print(f"\n{'='*60}")
print(f"✅ LYNGBY GLAS TEST TAMAMLANDI!")
print(f"{'='*60}")
print(f"📊 Sonuç:")
...
print(f"\n🎯 Şimdi Lyngby Glas markasını aç ve test et!")
