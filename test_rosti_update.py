...
# -*- coding: utf-8 -*-
"""
Rosti ürünlerini günceller
CSV: "RS 32144" → JSON: "32144"
"""
...
with open('gorsel_linkleri.csv', 'r', encoding='utf-8-sig') as f:
    ...
    for row in reader:
        ...
        # ROSTİ
        if 'ROST' in marka.upper() and sku and image_url:
            # SKU normalizasyonu: "RS 32144" → "32144" (prefix kaldır)
            sku_normalized = sku.replace('RS ', '').strip()
...
print(f"\n✅ {len(image_map)} Rosti ürün resmi bulundu (kayıt: {len(image_map)//2})\n")
...
csv_count = len([k for k in image_map.keys() if k.startswith('RS')])
print(f"\n{'='*60}")
print(f"✅ ROSTI TEST TAMAMLANDI!")
print(f"{'='*60}")
print(f"📊 Sonuç:")
...
print(f"\n🎯 Şimdi Rosti markasını aç ve test et!")
