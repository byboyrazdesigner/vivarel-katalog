...
# -*- coding: utf-8 -*-
"""
Gense ürünlerini günceller
CSV: "GN 34296" → JSON: "34296"
"""
...
with open('gorsel_linkleri.csv', 'r', encoding='utf-8-sig') as f:
    ...
    for row in reader:
        ...
        # GENSE
        if 'GENSE' in marka.upper() and sku and image_url:
            # SKU normalizasyonu: "GN 34296" → "34296" (prefix kaldır)
            sku_normalized = sku.replace('GN ', '').strip()
...
print(f"\n✅ {len(image_map)} Gense ürün resmi bulundu (kayıt: {len(image_map)//2})\n")
...
csv_count = len([k for k in image_map.keys() if k.startswith('GN')])
print(f"\n{'='*60}")
print(f"✅ GENSE TEST TAMAMLANDI!")
print(f"{'='*60}")
print(f"📊 Sonuç:")
...
print(f"\n🎯 Şimdi Gense markasını aç ve test et!")
