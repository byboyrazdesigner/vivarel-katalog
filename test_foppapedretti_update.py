...
# -*- coding: utf-8 -*-
"""
Sadece Foppapedretti ürünlerini günceller (TEST)
"""
...
with open('gorsel_linkleri.csv', 'r', encoding='utf-8-sig') as f:
    ...
    for row in reader:
        ...
        # Sadece Foppapedretti
        if 'FOPPA' in marka.upper() and sku and image_url:
            image_map[sku] = image_url
            print(f"  ✓ {sku}: {image_url[:60]}...")

print(f"\n✅ {len(image_map)} Foppapedretti ürün resmi bulundu\n")
...
# Sadece Foppapedretti ürünlerini güncelle
updated = 0
not_found = []

for product in products:
    if product.get('brand') == 'Foppapedretti':
...
print(f"\n{'='*60}")
print(f"✅ FOPPAPEDRETTI TEST TAMAMLANDI!")
print(f"{'='*60}")
print(f"📊 Sonuç:")
...
print(f"\n🎯 Şimdi Foppapedretti markasını aç ve test et!")
